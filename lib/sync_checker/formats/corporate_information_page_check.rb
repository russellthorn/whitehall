    ...
    class CorporateInformationPageCheck < EditionBase
      def expected_details_hash(corporate_information_page, _)
        super.tap do |details|
          ...
          #organisation... relation on model... pass in somehow ::::: edition_expected_in_live instead of passing in
          details.merge!(expected_corporate_information_groups(corporate_information_page)) if corporate_information_page.about_page?
        end
      end
      ...
      def expected_corporate_information_page_content_ids
        published_corporate_information_pages = edition_expected_in_live
          .organisation
          .corporate_information_pages
          .published

        [
          published_corporate_information_pages
            .by_menu_heading(:jobs_and_contracts)
            .map(&:content_id),
          published_corporate_information_pages
            .by_menu_heading(:our_information)
            .map(&:content_id),
          published_corporate_information_pages
            .for_slug('about-our-services')
            .try(:content_id)
          published_corporate_information_pages
            .for_slug('personal-information-charter')
            .try(:content_id)
          published_corporate_information_pages
            .for_slug('publication-scheme')
            .try(:content_id)
          published_corporate_information_pages
            .for_slug('social-media-use')
            .try(:content_id)
          published_corporate_information_pages
            .for_slug('welsh-language-scheme')
            .try(:content_id)
        ].compact
      end
